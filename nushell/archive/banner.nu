...
#     match $startup_time {
#         $t if $t == 0sec => null
#         $t if $t < 100ms => ($t | into string | paint green)
#         $t if $t < 500ms => ($t | into string | paint yellow)
#         $t => ($t | into string | paint red)
#     }
# }

# def uptime []: nothing -> string {
#     match (sys host).uptime {
#         $t if $t < 1day => ($t | round duration min | into string | paint green)
#         $t if $t < 1wk => ($t | round duration hr | into string | paint yellow)
#         $t => ($t | round duration day | into string | paint red)
#     }
# }

# def header_text []: nothing -> list<string> {
#     let curr_version = match (version check) {
#         $c if $c.current => ($"v($env.NU_VERSION)" | paint green)
#         $c => ($"v($env.NU_VERSION)" | paint yellow)
#     }
#     # let curr_version = $env.NU_VERSION | paint green
#     let shell = ("Nushell " | paint green) + $curr_version
#     let username = $env.USERNAME | paint light_purple
#     let hostname = sys host | get hostname | paint light_purple
...
#     my-ellie | paint green | row -s 0 -a c (header_text | contain -p t --pad-top 1 --pad-right 2) | contain -p tight
# }
...
#         mem_disks => (header | append $"("RAM" | paint blue): (status memory | get RAM)" | append (status disks | items {|mount status| $"($mount | paint blue): ($status)"}) | contain -a l | box)
...
#         }
#     }
#     # header | append $"("RAM" | paint blue): (status memory | get RAM)" | append (status disks | items {|mount status| $"($mount | paint blue): ($status)"}) | contain -a c | box
# }
